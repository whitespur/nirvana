...
#[derive(Accounts)]
pub struct SetNirvLoanOriginationFee<'info> {
...
pub fn handler(ctx: Context<SetNirvLoanOriginationFee>, fee: CoarseNumber) -> Result<()> {
    let config = &mut ctx.accounts.config;

    config.nirv_loan_origination_fee = fee;
