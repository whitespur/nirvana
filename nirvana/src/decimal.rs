...
/// TODO - unify the number system in Nirvana
/// There are too many different ways of doing math

pub const PRICE_SCALE: u8 = 8;
pub const UNIFIED_PERCENT_SCALE: u8 = 5;
pub const INTEREST_RATE_SCALE: u8 = 6;
pub const XUSD_SCALE: u8 = 6; // Remove leftovers
pub const ANA_SCALE: u8 = 6;
pub const STAKED_ANA_SCALE: u8 = 6;

// Risk free value coefficient
pub const RFV_SCALE: u8 = 6;

// Bond-control variable
pub const BCV_SCALE: u8 = 6;

#[zero_copy]
#[derive(PartialEq, PartialOrd, Default, Debug, AnchorDeserialize, AnchorSerialize)]
pub struct Decimal {
    // 17
    pub val: u128, // 16
    pub scale: u8, // 1
...
impl Decimal {
    ...
    pub fn to_rust_decimal(self) -> RDecimal {
        RDecimal::new(self.val.try_into().unwrap(), self.scale.into())
    }
    ...
    pub fn from_ana(val: u128) -> Decimal {
        Decimal {
            val,
            scale: ANA_SCALE,
        }
    }
    pub fn from_staked_ana(val: u128) -> Decimal {
        Decimal {
            val,
            scale: STAKED_ANA_SCALE,
        }
    }
    pub fn from_rfv(val: u128) -> Self {
        Decimal {
            val,
            scale: RFV_SCALE,
        }
    }
    pub fn from_bcv(val: u128) -> Self {
        Decimal {
            val,
            scale: BCV_SCALE,
        }
    }
    pub fn from_spl_token(account: TokenAccount, mint: Mint) -> Self {
        Decimal {
            val: account.amount.into(),
            scale: mint.decimals,
        }
    }
    ...
    pub fn to_ana(self) -> Decimal {
        self.to_scale(ANA_SCALE)
    }
...
mod test {
    ...
    fn test_to_interest_rate() {
        ...
        {
            {
                let decimal = Decimal::new(2222, 8);
                let actual = decimal.to_interest_rate();
                let expected = Decimal::new(22, INTEREST_RATE_SCALE);
...

            {
                ...
                let actual = decimal.to_interest_rate();
                let expected = Decimal::new(0, INTEREST_RATE_SCALE);
        ...
        {
            let decimal = Decimal::new(2222, INTEREST_RATE_SCALE);
            let actual = decimal.to_interest_rate();
            let expected = Decimal::new(2222, INTEREST_RATE_SCALE);
        ...
        {
            let decimal = Decimal::new(2222, 5);
            let actual = decimal.to_interest_rate();
            let expected = Decimal::new(22220, INTEREST_RATE_SCALE);
